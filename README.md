...
Serve the directory (paths are root-absolute, so opening the file directly won't load assets):
...
Bilingual via the `EN / 中` toggle. English lives at `/`, Chinese at `/zh/` — static text is generated from the `translations` table by `node scripts/build-zh.mjs` — both pages' body text, `<head>`, JSON-LD (incl. an FAQPage built from the FAQ entries) and `/llms.txt` — because share-card unfurlers and AI crawlers don't run JS, and a pre-commit hook in `.githooks/` rejects commits where it is out of date; the toggle switches in place and moves the URL with `history.replaceState`. All prose carries `data-i18n` / `data-i18n-html` / `data-i18n-svg` / `data-i18n-alt` attributes; the strings live in a `translations` object in `app.js` with an `en` + `zh` for every key. Code, command names and identifiers stay English in both modes. Preference persists in `localStorage`; first visit falls back to `navigator.language`; `?lang=en|zh` previews a language without saving it.
